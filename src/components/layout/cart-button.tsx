...
const CartButton: React.FC<Props> = ({ }) => {
    ...
    return (
        <button className='md:-ml-6 relative h-9 w-9'>
            ...
            <div className='absolute w-5 h-5 -right-2 bottom-0 bg-white flex items-center justify-center rounded-full text-sm text-sky-300 font-semibold'>{cartState?.length ?? 0}</div>
        </button>
    );
};
